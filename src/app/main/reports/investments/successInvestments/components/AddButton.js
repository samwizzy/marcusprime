...
class CustomToolbar extends React.Component {
  ...
  render() {
    const { classes, getInvestmentsByDate, allSuccessfulInvestments } = this.props;
    ...
    return (
      <React.Fragment>
        <Tooltip title={"Date filter"}>
          ...
          <Button type="submit" variant="contained" color="primary" onClick={() => getInvestmentsByDate(allSuccessfulInvestments, this.state)} className={classNames(classes.button, "mt-8 mb-16")}>
              Search
          </Button>
          </div>
        </Tooltip>
...
function mapStateToProps({ successInvestmentsApp, auth }) {
  return { 
