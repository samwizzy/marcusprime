...
class CustomToolbar extends React.Component {
  ...
  render() {
    const { classes, getTransferByDate, accounts, direction } = this.props;
    ...
    return (
      <React.Fragment>
        <Tooltip title={"Date filter"}>
          ...
          <Button type="submit" variant="contained" color="primary" onClick={() => getTransferByDate(accounts, this.state, direction)} className={classNames(classes.button, "mt-8 mb-16")}>
              Search
          </Button>
          </div>
        </Tooltip>
...
function mapStateToProps({ debitedCreditedApp, auth }) {
  return { 
