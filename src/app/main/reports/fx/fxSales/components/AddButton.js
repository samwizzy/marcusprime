...
class CustomToolbar extends React.Component {
  ...
  render() {
    const { classes, getFxSalesByDate, fxSales } = this.props;
    ...
    return (
      <React.Fragment>
        <Tooltip title={"Date filter"}>
          ...
          <Button type="submit" variant="contained" color="primary" onClick={() => getFxSalesByDate(fxSales, this.state)} className={classNames(classes.button, "mt-8 mb-16")}>
              Search
          </Button>
          </div>
        </Tooltip>
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators({
    getFxSalesByDate: Actions.getFxSalesByDate
  }, dispatch);
}

function mapStateToProps({ fxsalesApp, auth }) {
  return { 
