...
import { FormControlLabel, withStyles, Icon, Typography, Button } from '@material-ui/core';
...
class TBillsList extends Component {
    ...
    render() {
        const { maturitiesTbills, getInvestmentsByMaturity } = this.props;

        if(maturitiesTbills === null){
            return;
        }
        
        let TBills = _.filter(maturitiesTbills, item => {
            if(item.productCategory !== null){
                return item.productCategory.name === "Treasury Bills"
            }
        });
        ...
        const columns = [
            ...
            },
            {
                ...
                options: {
                    ...
                    customBodyRender: id => {
                        ...
                        const invest = TBills.find(TBill => id === TBill.id)
            ...
            },
            {
                ...
                options: {
                    filter: true,
                    customBodyRender: (value, tableMeta) => {
                        const maturity = TBills.find(maturity => value === maturity.id)
        ...
        const options = {
            ...
            downloadOptions: {filename: 'AllTbillsMaturityList.csv', separator: ','},
            onDownload: (buildHead, buildBody, columns, data) => {
                const categoriesIndex = columns.findIndex(c => c.label === "Amount");
                const datas = data.map(item =>  {
                    if (categoriesIndex > -1) {
                        const selectedMaturity = maturitiesTbills.find(mature => mature.id === item.data[categoriesIndex])
        ...
        return (
            <FuseAnimate animation="transition.slideUpIn" delay={300}>
                <MUIDataTable
                    title={"All Treasury Bills Running Maturities"}
                    data={TBills}
                    columns={columns}
                    options={options}
                />
