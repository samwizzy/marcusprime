...
class FxSalesRateList extends Component {
    ...
    render() {
        const { rates, openNewFxSwitchDialog, approveFixedRate, rights, user } = this.props;
        const selectedRates = rates && rates.filter(rate => rate.type === 'FX SALE')
        const r = _.find(rights, function(o) { return o.module.moduleName === 'Transactions'; });
        ...
        return (
            <React.Fragment>
                <FuseAnimate animation="transition.slideUpIn" delay={300}>
                    <MUIDataTable
                        title={"FX Sales Rate Guide ― Maker / Checker"}
                        data={selectedRates}
                        columns={columns}
                        options={options}
                    />
