...
class GBPToUSDLists extends Component {

    render()
    {
        const { fxswitch, filteredFxswitch, isFiltering } = this.props;
        const allFxswitch = isFiltering? filteredFxswitch : fxswitch
        const filterFxswitch = allFxswitch && allFxswitch.filter(fx => (fx.buyCurrency === 'GBP' && fx.sellCurrency === 'USD') || (fx.buyCurrency === 'USD' && fx.sellCurrency === 'GBP'))
        ...
        const options = {
            ...
            downloadOptions: {filename: 'GBPToUSDList.csv', separator: ','},
        ...
        return (
            <FuseAnimate animation="transition.slideUpIn" delay={300}>
                <MUIDataTable
                    title={"FX Switch Report (GBP/USD)"}
...
export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(GBPToUSDLists)));
