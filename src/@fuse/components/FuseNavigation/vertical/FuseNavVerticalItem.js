...
function FuseNavVerticalItem({item, classes, nestedLevel, role, authRoles, rights, navbarCloseMobile, active})
{
    // if ( item.auth && (!item.auth.includes(role.name.toLowerCase()) || (role.name.toLowerCase() !== 'guest' && item.auth.length === 1 && item.auth.includes('guest'))) )
    // // if ( authRoles && (!authRoles.includes(role.name.toLowerCase()) || (role.name.toLowerCase() !== 'guest' && authRoles.length === 1 && authRoles.includes('guest'))) )
    // {
    //     return null;
    // }
