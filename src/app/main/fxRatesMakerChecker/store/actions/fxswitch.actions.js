...
export const APPROVE_FIXED_RATES = '[FX SWITCH APP] APPROVE FIXED RATES';
...
{
    const request = axios.get('financialserv/api/v1/temp_rates');
...
{
    const request = axios.get('financialserv/api/v1/temp_rates/' + id);
...
{
    const request = axios.get('financialserv/api/v1/temp_rates/find_by_type', { params });
...
export function approveFixedRate(params) {
    const request = axios.post('financialserv/api/v1/temp_rate/approve', qs.stringify(params));

    return (dispatch) =>
        request.then((response) =>
            Promise.all([
                dispatch({
                    type: APPROVE_FIXED_RATES,
                    payload: response.data
                })
...
{
    const request = axios.post('financialserv/api/v1/temp_rates/add', data);
