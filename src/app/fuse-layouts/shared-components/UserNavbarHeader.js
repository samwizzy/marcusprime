...
const UserNavbarHeader = ({user, classes}) => {
    return (
        <AppBar
            ...
            className="user relative flex flex-col items-center justify-center pt-12 pb-32 mb-32 z-0"
        >
            <Typography className="username text-16 whitespace-no-wrap" color="inherit">{user.data.userName}</Typography>
...
function mapStateToProps({fuse, auth})
{
    return {
        // user: auth.user,
        user: auth.user.data.role && auth.user.data.role.name === 'guest'? auth.user : JSON.parse(localStorage.getItem('data')),
    }
}
