...
class DebitedCreditedSidebarContent extends Component {

    render()
    {
        const {accountTypes, getAllInvestments, getCreditsDebitsAccounts, classes } = this.props;
                ...
                <FuseAnimate animation="transition.slideLeftIn" delay={200}>
                    <Paper elevation={1} className="rounded-8">
                        <div className="p-24 flex items-center">
                            <Typography>All User Accounts</Typography>
                        </div>
                        <Divider/>
                        <List>
                            <ListItem
                            ...
                            </ListItem>
                            {
                                accountTypes.map(item =>
                                    ...
                                    <ListItem
                                        ...
                                        button
                                        component={NavLink}
                                        to={'/reports/accounts/' + item.direction}
                                        onClick={(ev) => {
                                            ev.stopPropagation();
                                            getCreditsDebitsAccounts({ direction: item.direction})
                                        }}
                                        activeClassName="active"
                                        className={classes.listItem}
                                    >
...
{
    return {
        accountTypes: [{id: 1, name: 'Account Debited', direction: 'debit'}, {id: 2, name: 'Account Credited', direction: 'credit'}]
    }
}

export default withStyles(styles, {withTheme: true})(withRouter(connect(mapStateToProps, mapDispatchToProps)(DebitedCreditedSidebarContent)));
