...
class CustomToolbar extends React.Component {
  ...
  render() {
    const { classes, getPaystackLogByDate, paystacklogs } = this.props;
    ...
    return (
      <React.Fragment>
        <Tooltip title={"Date filter"}>
          ...
          <Button type="submit" variant="contained" color="primary" onClick={() => getPaystackLogByDate(paystacklogs, this.state)} className={classNames(classes.button, "mt-8 mb-16")}>
              Search
          </Button>
          </div>
        </Tooltip>
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators({
    getPaystackLogByDate: Actions.getPaystackLogByDate
  }, dispatch);
}

function mapStateToProps({ paystackLogApp, auth }) {
  return { 
